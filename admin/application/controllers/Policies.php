<?php
    ...
    function add()
    {
		...
		if($this->input->post('action') == 'add_policies') 
		{ 
			...
			$this->policies_model->add($data);
			$this->session->set_flashdata('L_strErrorMessage','Policies Added Successfully');
			redirect($this->config->item('base_url').'policies/lists');
    }

    $data['compare_name'] = $this->policies_model->all_compare("compare_name");
    $data['all_title'] = $this->policies_model->all_compare("title");
	$data['all_company_name'] = $this->policies_model->alldata("company_name");		$data['policy_category'] = $this->policies_model->alldata("policy_category");
    $data['all_sum_insured'] = $this->policies_model->all_sum_insure("sum_insured");
    $this->load->view('add_policies',$data);
}
	...
	{	 
			if(is_numeric($id))
			{
				$result = $this->policies_model->is_exist($id); 
                //print_r($result);exit(); 
				$form_field = $data = array(

						'L_strErrorMessage' => '',
						'id'	=> $result->id,												'cat_id'	=> $result->cat_id,
				...
				if($this->input->post('action') == 'edit_policies') 
				{
                    ...
                    {  
						...
						$this->policies_model->edit($id, $form_field);
						$this->session->set_flashdata('L_strErrorMessage','Policies Updated Successfully');
						redirect($this->config->item('base_url').'policies/lists');
					}
				$data['addition_item'] = $this->policies_model->addition_item($id);
                $data['exclusion_item'] = $this->policies_model->exclusion_item($id);
                $data['top_feature_item'] = $this->policies_model->top_feature_item($id);
                $data['sum_insuranc_data'] = $this->policies_model->sum_insuranced_attr($id);
                $data['all_company_name'] = $this->policies_model->alldata("company_name");								$data['policy_category'] = $this->policies_model->alldata("policy_category");
                $data['all_sum_insured'] = $this->policies_model->all_sum_insure("sum_insured");
                $data['compare_name'] = $this->policies_model->all_compare("compare_name");
                $data['all_title'] = $this->policies_model->all_compare("title");
                $data['addition_detail'] = $this->policies_model->addition_detail($id);
				$this->load->view('edit_policies',$data);
			} 
			else 
			{
	...
	function updateorder($id,$val){
        $this->policies_model->updateorder($id,$val);
	...
	function lists()
	{
		...
		$config['base_url'] = $url_to_paging.'policies/lists/';
		...
		$return = $this->policies_model->lists($config['per_page'],$this->uri->segment(3), $data);
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		//echo "<pre>";print_r($data);break;
		$this->pagination->initialize($config);
	$this->load->view('list_policies', $data);
	}
	...
	function deletes()
	{
        if(isset($_POST['selected']) && count($_POST['selected']) > 0) 
        {
            ...
            {
                if($this->policies_model->deletes($selCheck)) 
                {
	...
	function removeprice($coverage_id, $id) {
        if ($this->policies_model->removeattriute($coverage_id, $id)) {
            $this->session->set_flashdata('L_strErrorMessage', 'Coverage Deleted Succcessfully !');
            redirect($this->config->item('base_url') . 'policies/edit/' . $coverage_id);
        } else {
            $this->session->set_flashdata('flashError', 'Some Errors prevented from Deleting!');
            exit;
        }
    ...
    function remove_topFeature($policies_id, $id) {
        if ($this->policies_model->remove_topFeature($policies_id, $id)) {
            $this->session->set_flashdata('L_strErrorMessage', 'Top Feature Deleted Succcessfully !');
            redirect($this->config->item('base_url') . 'policies/edit/' . $policies_id);
        } else {
            $this->session->set_flashdata('flashError', 'Some Errors prevented from Deleting!');
            exit;
        }
    ...
    function remove_sum_insuranc($policies_id, $id) {
        if ($this->policies_model->remove_sum_insuranc($policies_id, $id)) {
    ...
    function remove_exclusion($exclusion_id, $id) {
        if ($this->policies_model->remove_exclusion($exclusion_id, $id)) {
            $this->session->set_flashdata('L_strErrorMessage', 'Exclusions Deleted Succcessfully !');
            redirect($this->config->item('base_url') . 'policies/edit/' . $exclusion_id);
        } else {
            $this->session->set_flashdata('flashError', 'Some Errors prevented from Deleting!');
            exit;
        }
    ...
    function remove_compare($compare_id, $id) {
        if ($this->policies_model->remove_compare($compare_id, $id)) {
            $this->session->set_flashdata('L_strErrorMessage', 'Compare Deleted Succcessfully !');
            redirect($this->config->item('base_url') . 'policies/edit/' . $compare_id);
        } else {
            $this->session->set_flashdata('flashError', 'Some Errors prevented from Deleting!');
            exit;
        }
    ...
    function updateorder_status($id, $value) {
        $return = $this->policies_model->updateorder_status($id, $value);
    ...
    function show_title(){
        ...
        $data = $this->policies_model->show_title($id);
