<?php
	...
	function add()
	{
		...
		if($this->input->post('action') == 'add_compare') 
		{
			...
			$this->validation->set_fields($fields);
						 ...
						 $this->compare_model->add($data);
						$this->session->set_flashdata('L_strErrorMessage','Compare Added Successfully');
						redirect($this->config->item('base_url').'compare/lists');
						if ($this->validation->run() == FALSE){
					$data['L_strErrorMessage'] = $this->validation->error_string;
					} 
		}
		
		$this->load->view('add_compare',$data);
	}
    function edit($id)
	{	 
	//echo $id; die;
			if(is_numeric($id))
			{
				$result = $this->compare_model->get_compare($id);  
				//print_r($result);die();
					$form_field = $data = array(
						'L_strErrorMessage' => '',
						'id'	=> $result->id,
				...
				if($this->input->post('action') == 'edit_compare') 
				{
				//	echo $id; die;
					...
					// 	}
							$this->compare_model->edit($id, $form_field);
							$this->session->set_flashdata('L_strErrorMessage','Compare Updated Successfully');
							redirect($this->config->item('base_url').'compare/lists');
					}
				}
				
				$this->load->view('edit_compare',$data);
			} 
			else 
			{
				$this->session->set_flashdata('L_strErrorMessage','No Such Compare !!!!');
	...
	function lists()
	{
		...
		$config['base_url'] = $url_to_paging.'compare/lists/';
		...
		$return = $this->compare_model->lists($config['per_page'],$this->uri->segment(3), $data);
		$data['result'] = $return['result'];
		$config['total_rows'] = $return['count'];
		// echo "<pre>";print_r($data);exit;
		$this->pagination->initialize($config);
		$this->load->view('list_compare', $data);
	}
	function deletes()
	{
		if(isset($_POST['selected']) && count($_POST['selected']) > 0) {
			foreach($_POST['selected'] as $selCheck) {
				if($this->compare_model->deletes($selCheck)) {
					$this->session->set_flashdata('L_strErrorMessage','Compare Deleted Successfully');
				}  
				else 
				{
