<?php
	...
	function add($content) 
	{
		...
		$this->_data = $data;
		if ($this->db->insert('blog_category', $this->_data))	
		{		
			return true; 
		} 
		else 
...

	{
		...
		if ($this->db->update('blog_category', $this->_data))	{
	...
	{
		$query = "update blog_category set set_home = '".$value."' where id = '".$pid."'";
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql_count = "SELECT * FROM  blog_category  WHERE id <> 0";
	...
	function updateorder($id,$val){
		...
		return $this->db->update('blog_category', $content);	
