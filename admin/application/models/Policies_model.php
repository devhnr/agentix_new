<?php
	...
	function add($content)
 	{	
       ...
       $data['coverage_detail'] = $content['coverage_detail'];
		...
		$this->_data = $data;
		if ($this->db->insert('policies', $this->_data))
			{	
	...
	function edit($id, $content) 
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
        if ($this->db->update('policies', $this->_data))	
        {	 
	...
	function updateorder($id,$val){
		...
		return $this->db->update('policies', $content);	
	}
    ...
    function lists($num, $offset, $content) 
	{
		...
		$sql_count = "SELECT * FROM policies WHERE id <> 0";
    ...
    function updateorder_status($id, $val) {
        $query = "update policies set status = '" . $val . "'  where id = '" . $id . "'";
    ...
    }		function get_category_name($id) {        $this->db->where('id = ', $id);        $query = $this->db->get('policy_category');        if ($query->num_rows() > 0) {            $result = $query->row()->name;            return $result;        } else {            return false;        }    }
