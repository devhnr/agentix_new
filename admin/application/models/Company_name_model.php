<?php
	class Company_name_model extends CI_Model {
	private $_data = array();
...

	{
		...
		if ($this->db->insert('company_name', $this->_data))	
...

	{
		...
		if ($this->db->update('company_name', $this->_data))	{
	...
	{
		$query = "update company_name set set_home = '".$value."' where id = '".$pid."'";
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM company_name where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  company_name  WHERE id <> 0";
...

	{
		...
		if ($this->db->delete('company_name'))	{
	...
	function updateorder($id,$val){
		...
		return $this->db->update('company_name', $content);	
