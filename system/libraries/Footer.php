...
class CI_Footer {
	   ...
	   function allsubscribe_head()
		...
		function allcraft()
		{
			$sql = "SELECT * FROM fabric order by set_order asc";
			$query = $this->CI->db->query($sql);
			if ($query->num_rows() > 0)
			{
				$result = $query->result();
		...
		function collection()
		{
			$sql = "SELECT * FROM menu_collection order by set_order asc";
			$query = $this->CI->db->query($sql);
			if ($query->num_rows() > 0)
			{
				$result = $query->result();
		...
		{
			$sql = "select count(*) as totalWishlist from wishlist where userid = ".$uid;
			$query = $this->CI->db->query($sql);
			if ($query->num_rows() > 0)
			{
				$result = $query->row();
