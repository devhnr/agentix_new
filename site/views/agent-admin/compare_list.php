                ...
                <div class="col-lg-4">
                    ...
                    ?>
                        ...
                        <div class="insur-cta">
                            ...
                            <!-- <input type="checkbox" id="comparein_<?php echo $all_policy_data->id;?>"  name="" class="compare-in"> <label for="comparein_<?php echo $all_policy_data->id;?>" id="comparelabel" class="compare-btn">Add to Compare</label> -->
                            <input type="checkbox" id="comparein_<?php echo $all_policy_data->id;?>"  name="" class="compare-in"> <label for="comparein_<?php echo $all_policy_data->id;?>" id="comparelabel" class="compare-btn" onclick="compare_policy('<?php echo $all_policy_data->id;?>','<?php echo $premium_amount_filter;?>','<?php echo $sum_insured_filter;?>','<?php echo $all_policy_data->plan_name;?>','<?php echo $all_policy_data->cat_id;?>');">Add to Compare</label>
                        </div>
                    </div>
                </div>
            <?php }  }else{?>
...
<?php } } ?>
<div class="com-add-sec">
    <div class="container-fluid">
        <!--<div class="row">-->
        <!--    <div class="col-lg-12">-->
        <!--        <h4 class="mb-3">Compare</h4>-->
        <!--    </div>-->
        <!--</div>-->
<div id="mydiv_pc_new">
        <div class="row align-items-end">
            <?php
            //echo"<pre>";print_r($this->session->userdata('compare_array'));echo"</pre>";
           $compareArray = $this->session->userdata('compare_array');

            if (isset($compareArray) && !empty($compareArray)) {
                
            foreach($this->session->userdata('compare_array') as $key => $value){
                $explode = explode('|',$value);
                $policies_data = $this->agent_admin_model->get_policies_new($explode[0],$explode[1]);
                $company_logo = $this->agent_admin_model->get_company_image($policies_data->company_id);
                $company_data = $this->agent_admin_model->get_company_data($policies_data->company_id);
                //echo"<pre>";print_r($company_data);echo"</pre>";
            ?>
            <div class="col-lg-3">
                <div class="com-add-col">
                    <button type="button" class="close" onclick="remove_compare_data('<?php echo $base_url; ?>agent_admin/delete_compare/<?php echo $explode[0]; ?>/<?php echo $explode[1]; ?>/<?php echo $explode[2]; ?>/<?php echo $policy_cat_data->page_url;?>');">
            <span aria-hidden="true">×</span>
          </button>
          <div class="com-add-flex">
               <div class="com-add-img">
                        <?php if($company_logo !=''){?>
                            <img src="<?php echo $http_host?>upload/policies_image/medium/<?php echo $company_logo?>" alt="">
                        <?php } else {?>
                            <img src="<?php echo $http_host?>upload/policies_image/medium/noimage.jpg">
                    ...
                    <div class="com-add-title">
                        <h6><?php echo $company_data->name?></h6>
                        <p><?php echo $policies_data->plan_name?></p>
                    </div>
          </div>
                    <!-- <div class="com-add-rate">-->
                    <!--    <h6>₹ 13000/-</h6>-->
                    <!--</div>-->
                </div>
            </div>
        <?php } ?>
          ...
          <div class="com-add-flex">
               <div class="com-add-img">
                        <img src="https://agentix.in/beta/upload/policies_image/medium/1694435213.ICICI_Lombard.png" alt="">
                    </div>
                    <div class="com-add-title">
                        <h6>Icici Lombard</h6>
                        <p>Complete Health Insurance - Health Shield Plus</p>
                    </div>
          </div>
                </div>
            </div>
          ...
          <div class="com-add-flex">
            ...
            <div class="col-lg-3">
                <!-- <button type="button" class="btn com-add-sub">Compare</button> -->
                <a href="<?php echo $base_url?>agent-admin/compare" class="btn com-add-sub" style="
    width: 100%;
    text-align: center;
...
">Compare Now <i class="feather icon-feather-chevrons-right"></i></a>
             ...
             <?php }else { ?>
                <p><?php echo "Please add policies to compare"; ?></p>
                <?php } ?>
        </div>
    </div>
    </div>
...
<?php
    if($this->session->userdata('compare_array_count') == ''){
        $compare_array_count = 0;
    }else{
        $compare_array_count = $this->session->userdata('compare_array_count');
    }
?>
...
function compare_policy(policy_id,premium_amount,sum_insured,plan_name,cat_id){

    if(<?php echo $compare_array_count;?> >= 3){
        alert('You Can select Max Three Policies to compare');
    ...
    $.ajax({
        ...
        success : function(msg){
            ...
            $("#mydiv_pc_new").load(location.href + " #mydiv_pc_new");
