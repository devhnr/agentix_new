...
use PHPMailer\PHPMailer\Exception;
    ...
    $this->load->library("pagination");
  	...
  	function index(){ 
		$data['error'] = "";
		$data['metatitle'] = 'Agentix Admin | Dashboard';
	...
	function product_comparison(){ 
		...
		$data['metatitle'] = 'Agentix Admin | Product Comparision';
	...
	function compare_list($page_url){ 
		...
		$data['get_sum_insured_detail'] = $this->agent_admin_model->get_sum_insured_detail();
		 ...
		 if($data['policy_cat_data']->id != $this->session->userdata('compare_array_cat_id')){
	        $this->session->unset_userdata('compare_array');
	         $this->session->unset_userdata('compare_array_count');
	    }	
		
		$data['metatitle'] = 'Agentix Admin | Compare Lists';
		//echo"<pre>";print_r($data);echo"</pre>";exit;
    ...
    function compare_policy(){
    	...
    	$this->session->set_userdata('compare_array_cat_id',$cat_id);
	...
	function compare(){
		$data['error'] = "";
		$data['metatitle'] = 'Agentix Admin | Compare';
	...
	function product_respository($page_url){
		...
		$data['all_company_name'] = $this->agent_admin_model->all_company_name();
		$data['metatitle'] = 'Agentix Admin | Product Repository Lists';
		//echo "<pre>";print_r($data);echo"</pre>";exit;
	...
	function repository_of_brochure(){ 
		$data['error'] = "";
		$data['policy_category'] = $this->agent_admin_model->get_all_policy_category();
		$data['metatitle'] = 'Agentix Admin | Product Repository';
		//echo"<pre>";print_r($data);echo"</pre>";exit;
...
// error_reporting(E_ALL);
        ...
        $claim_form_data = $this->agent_admin_model->policy_repository_data($_POST['policy_id'],'4');

        $AddAttachment = array();

        if($policy_wording_data->document_file != ''){

        	$file_wording = $this->config->item('base_url').'upload/product_repository/'.$policy_wording_data->document_file;

        	$AddAttachment[] = $file_wording;
        }

        if($policy_brochure_data->document_file != ''){

        	$file_wording = $this->config->item('base_url').'upload/product_repository/'.$policy_brochure_data->document_file;

        	$AddAttachment[] = $file_wording;
        }

        if($claim_form_data->document_file != ''){

        	$file_wording = $this->config->item('base_url').'upload/product_repository/'.$claim_form_data->document_file;
...
// error_reporting(E_ALL);
	...
	function new_products(){  
		...
		//echo "<pre>";print_r($data);echo"</pre>";exit;
		$data['metatitle'] = 'Agentix Admin | New Products';
	...
	function client_educational_content(){  
		...
		//echo "<pre>";print_r($data);echo"</pre>";exit;
		$data['metatitle'] = 'Agentix Admin | Clients Educational Content';
	...
	function recommended_poster(){  
		$data['error'] = "";
		$data['recommended_poster'] =$this->agent_admin_model->get_recommended_poster();
		$data['metatitle'] = 'Agentix Admin | Recommended Poster';
	...
	function daily_poster(){  
		$data['error'] = "";
		$data['daily_poster'] =$this->agent_admin_model->get_daily_poster();
		$data['metatitle'] = 'Agentix Admin | Daily Poster';
	...
	function article_poster(){  
		$data['error'] = "";
		$data['all_blogs'] =$this->agent_admin_model->get_all_blogs();
		$data['metatitle'] = 'Agentix Admin | Article Poster';
